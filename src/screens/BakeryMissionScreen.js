...
import { bakeryFoodsLesson } from "../data/bakeryFoodsLesson";

export default function BakeryMissionScreen({ navigation, route }) {
  const autoStart = route?.params?.autoStart;
  const lessonId = route?.params?.lessonId ?? bakeryFoodsLesson.id;
  const [questionIndex, setQuestionIndex] = useState(0);
  const [selectedOptionIndex, setSelectedOptionIndex] = useState(null);
  const [isAnswerCorrect, setIsAnswerCorrect] = useState(false);
  const [feedbackMessage, setFeedbackMessage] = useState(bakeryFoodsLesson.introMessage);
  const [isCompleting, setIsCompleting] = useState(false);

  const questions = bakeryFoodsLesson.questions;
  ...
  const handleNextQuestion = () => {
    ...
    setFeedbackMessage(bakeryFoodsLesson.introMessage);
  };
  ...
  const handleCompleteMission = async () => {
    ...
    try {
      await markLocationCompleted("bakery");
      await markLessonCompleted({ lessonId, locationId: "bakery" });
      navigation.goBack();
    } catch (_error) {
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", padding: 20 }}>
        <View style={{ backgroundColor: "white", borderRadius: 24, padding: 20, borderWidth: 3, borderColor: "#FFB300" }}>
          ...
          <Text style={{ fontSize: 28, fontWeight: "bold", marginBottom: 8, textAlign: "center", color: "#E65100" }}>
            {bakeryFoodsLesson.title}
          </Text>
          <Text style={{ fontSize: 16, color: "#444", textAlign: "center", lineHeight: 22, marginBottom: 14 }}>
            {autoStart
              ? "Licao iniciada! Vamos aprender comidas da padaria em ingles."
              : "Bem-vindo a Padaria!"}
          </Text>
